...
7. ✅ Leaves a **bare git mirror** under `/tmp/tron-scans/` so **re-audits and UI retries** keep working (see cleanup below)
...
4. **Git mirror on disk**
   - Removes the **working-tree** copy after the bare repo is built (saves space)
   - **Keeps** `tron-scan-bare-*.git` because the Tron project’s `repo_url` is `file://…` pointing at it
   - **Deleting that directory** (or macOS clearing `/tmp`) causes clone errors on the next run — recreate with this script or point the project at a new URL

---

## Retries and “does not appear to be a git repository”

If an audit fails with:

`fatal: '.../tron-scan-bare-….git' does not appear to be a git repository`

common causes are:

1. **Stale `file://` URL** — an older version of the helper script deleted the bare repo after a successful scan; the project still referenced it. **Fix:** run `scripts/scan_local_folder.sh` again (new project), or **PUT** `/api/projects/{id}` with a fresh `repo_url`, or restore the bare repo from backup.
2. **Host `/tmp` cleared** — Docker mounts host `/tmp/tron-scans` into the worker; if the bare repo was removed, clones fail the same way.
3. **Wrong default branch** — the script now sets `default_branch` from the local repo (`main` vs `master`).

To **remove** a mirror when you no longer need the project:

```bash
rm -rf /tmp/tron-scans/tron-scan-bare-*.git
```

(Only after changing or deleting the Tron project that pointed at that path.)
