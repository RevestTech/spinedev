...
**Documentation map:** **`docs/README.md`** (folder tree) and **`docs/BLUEPRINT.md`** (full index). This file is the **evidence index** for requirements.

## Sources of truth (in order)

1. **`docs/archive/PROPOSAL.md`** — product intent and full feature set (historical contract).
2. **`docs/project/BRD.md`** — business outcomes delivered vs **`PROPOSAL.md`** (completed capability narrative).
3. **`docs/project/TRD.md`** — technical architecture pointers and verified-delivery references.
4. **`docs/project/MASTER_PROPOSAL_TODO.md`** — **remaining open backlog** vs the proposal only (must stay current).
5. **This file** — rules for marking items **done**, **partial**, or **out of scope**, plus the **verified deliveries** evidence table.
...
**Rule:** If something is **Partial**, the limitation must appear in **`MASTER_PROPOSAL_TODO.md`** (open backlog), **`BRD.md`**, or an ADR—not only in chat.
...
| **ISO prompt / schema regression CI** | **`.github/workflows/prompt-regression.yml`** runs **`pytest tests/golden_suite`** on a daily schedule and on PR/push when **`tron/agents/**`**, **`tron/schemas/**`**, or **`tests/golden_suite/**`** change. Deterministic (mocked LLM fixtures); see workflow header comments. |
...
| **Security audit: trust & evidence (API/UI/export)** | **Done** — SARIF merge, path filters, triage, provenance, copy, **`follow_up_recommended`** flags, optional **SEC-5 second sandbox pass** when **`TRON_DEEP_VERIFY_TOP_N` > 0** — Temporal **`deep_verify_follow_up_findings`** (**`tron/workflows/audit_workflow.py`**, **`tron/workflows/activities.py`**); in-process **`apply_deep_verify_retry_pass_to_outputs`** (**`tron/services/layer3_findings.py`**, **`audit_executor.py`**). Evidence: **`tests/unit/test_layer3_findings.py`**, **`tests/unit/test_workflow_handlers.py`**. |
...
Proposal-aligned gaps: **none** (see **`docs/project/MASTER_PROPOSAL_TODO.md`**). Certified attestation packs are **Deferred** (ADR-002), not **Partial**.

**Resume work:** open **`docs/project/MASTER_PROPOSAL_TODO.md`** for **open backlog rows**; use **`docs/project/BRD.md`** / **`docs/project/TRD.md`** for delivered scope; use **this file** for evidence pointers per requirement.
...
1. Any change that **closes an open backlog row** updates **`MASTER_PROPOSAL_TODO.md`**, **`BRD.md`§7**, **`TRD.md`§8** as applicable, and **this file**, in the same PR/commit.
2. New scope decisions use **`docs/architecture/`** or a short ADR under **`docs/project/`** when they override the proposal.
3. **Infrastructure and deployment hardening** (TLS, CI, CORS, key storage, etc.) are tracked in **`docs/project/HARDENING_REVIEW_TODO.md`**; they are not proposal features but affect production readiness.
