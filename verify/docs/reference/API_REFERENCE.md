...
Human-maintained companion to the live OpenAPI spec (**`/api/openapi.json`**, Swagger at **`/api/docs`**). Application version matches **`tron/api/main.py`** metadata.
...
Most endpoints require **`X-API-Key`** (CLI and automation). The primary SPA also authenticates via **httpOnly cookie** after **`POST /api/admin/login`**; the server accepts that session cookie where **`require_api_key`** is used (same scopes as keys).
...
### POST /api/audits/diff

**Start a diff-scoped audit** (PR gate): same pipeline as a full audit, but **`AuditRun.diff_files_json`** restricts scanned files to the caller-supplied relative paths (typically from `git diff --name-only`). Git refs are informational only—the API does not run git.
...
```json
{
  ...
  "branch": "main",
  "commit_hash": "optional sha",
  "trigger_type": "pr"
}
```

- **`changed_files`**: non-empty list (max 2000 entries); each path must be relative repo paths with no `..` segments (validated server-side).

**Response (201 Created):** Audit summary (`AuditSummary`), same shape as **`POST /api/audits`**.

**Implementation:** `tron/api/routes/audits.py` (`AuditDiffCreate`, `create_diff_audit`).
...
**Finding statuses:** Primarily **`open`** (default); dismiss flow sets **`dismissed`** with **`resolution`** / **`resolved_at`** (see Finding triage routes below).
...
**Dismiss a finding** and record a **project-level fingerprint suppression** so it does not reappear on the next audit (`FindingSuppression`).

**Request body:**
```json
{ "reason": "false positive — intentional test fixture (required)" }
```
...
### POST /api/findings/{finding_id}/restore

**Re-open a dismissed finding** and remove its fingerprint from the project suppression set.
...
### GET /api/projects/{project_id}/finding-suppressions

**List fingerprint suppressions** for a project.

**Response:** JSON array of `{ project_id, fingerprint, reason, created_at }`.

**Auth:** Required (X-API-Key)

---

### DELETE /api/projects/{project_id}/finding-suppressions/{fingerprint}

**Remove a suppression** by fingerprint (findings with that fingerprint may show again on future audits).
