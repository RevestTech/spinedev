...
# ── Spine v3 bootstrap (one-command cold-start) ─────────────────────
...
.PHONY: bootstrap bootstrap-clean doctor nuke flyway-sync smoke hygiene hygiene-apply lint lint-py lint-html lint-sql lint-md lint-fix

bootstrap: ## Cold-start the whole Spine stack (preflight → venv → pg → migrations → smoke)
...
smoke: ## Run tools/smoke-test.sh (Spine integration smoke harness)
...
PY_LINT_FILES := db/watcher/spine_watcher.py db/dashboard/build-snapshot.py db/dashboard/serve.py db/dashboard/tests/test_approval.py migration/_v1_v2_migrator_legacy.py
...
MD_LINT_FILES := CHANGELOG.md INSTALL.md README.md docs/SPINE_MASTER.md docs/PRD.md docs/V1_SHIP_CHECKLIST.md
