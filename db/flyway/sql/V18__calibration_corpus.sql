...
CREATE TABLE spine_calibration.outcome (
    ...
    CONSTRAINT outcome_source_chk CHECK (outcome_source IN
...
SELECT c.role, c.output_type, c.labeled_samples, c.predictions,
...
FROM   counts c
LEFT JOIN spine_calibration.calibration_model m
...
SELECT p.id, p.role, p.output_type, p.project_id, p.subject_id,
       p.predicted_value, p.predicted_at,
