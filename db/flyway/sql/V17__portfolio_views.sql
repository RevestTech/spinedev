...
SELECT  id AS project_id, project_uuid AS uuid, name, current_phase, status,
...
SELECT  rh.project_id, p.name AS project_name, p.current_phase AS project_phase,
        rh.phase AS dispatched_phase, rh.subsystem, rh.role, rh.directive_ref,
        rh.dispatched_at,
...
SELECT
    (SELECT COUNT(*) FROM spine_lifecycle.project WHERE status='active')::bigint AS active_projects,
    ...
    (SELECT COUNT(*) FROM spine_lifecycle.portfolio_queue
       WHERE dispatched_at IS NULL)::bigint                                      AS queued_directives,
    COALESCE((SELECT SUM(cost_usd) FROM spine_recording.costs
               WHERE ts >= date_trunc('day',  NOW())), 0)::numeric               AS spend_today_usd,
    COALESCE((SELECT SUM(cost_usd) FROM spine_recording.costs
               WHERE ts >= date_trunc('month',NOW())), 0)::numeric               AS spend_month_usd,
    NOW()                                                                        AS computed_at;
...
SELECT
    p.id AS project_id, p.project_uuid AS uuid, p.name, p.current_phase, p.status,
    ...
    (SELECT COUNT(*) FROM spine_lifecycle.route_history rh
       WHERE rh.project_id = p.id AND rh.completed_at IS NULL)::bigint AS in_flight,
    (SELECT COUNT(*) FROM spine_lifecycle.portfolio_queue q
       WHERE q.project_id = p.id AND q.dispatched_at IS NULL)::bigint  AS queue_depth,
    COALESCE((SELECT SUM(c.cost_usd) FROM spine_recording.costs c
               WHERE c.project_id = p.id
                 AND c.ts >= date_trunc('day', NOW())), 0)::numeric    AS cost_today_usd
